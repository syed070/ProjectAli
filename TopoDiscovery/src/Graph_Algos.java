/**
 * Created by amuhamma on 7/7/2017.
 */
...
public class Graph_Algos
{
    ...
    public HashMap<String,Double> BetweenNess_Centrality_Score(LinkedList<String> Distinct_nodes, LinkedList<String> source_vertex, LinkedList<String> target_vertex, LinkedList<Double> Edge_Weight)
    {

        HashMap<String,Double> cbNodes = new HashMap<String,Double>();
...
//CREATING weighted directed graph
        ...
        for(int i=0;i<Graph_Nodes_Only.size();i++)
        {
            ...
            cbNodes.put(Graph_Nodes_Only.get(i).toString(),BC1.getVertexScore(Graph_Nodes_Only.get(i)));
        }
        return cbNodes;
    }

    public HashMap<String,Double> graphInput(ArrayList<Node> nodeList, ArrayList<Nodal_Links> demandMatrix)
    {
        HashMap<String,Double> cbNodes = new HashMap<String,Double>();
...
//add the distinct vertexes
        ...
        for(Node n:nodeList){
            Distinct_Vertex.add(n.getNodeName());
        }
...
//        Distinct_Vertex.add("G");
         ...
         for(Nodal_Links nl:demandMatrix){
             Source_Vertex.add(nl.Src.getNodeName());
             Target_Vertex.add(nl.Dst.getNodeName());
             Edge_Weight.add(nl.bandwidth);
         }
...
//        Source_Vertex.add("G"); Target_Vertex.add("F"); Edge_Weight.add(0.2);
   //     System.out.println("Graph \n " + g);
       ...
       cbNodes = GA1.BetweenNess_Centrality_Score(Distinct_Vertex, Source_Vertex, Target_Vertex, Edge_Weight);
